...
function docSnapshotObservable(doc, options) {
    return new rxjs_1.Observable(subscriber => {
        if (union_types_1.DocumentReference.isClient(doc)) {
            const unsubscribe = doc.onSnapshot((0, extractSnapshotListenOptions_1.extractSnapshotListenOptions)(options) || {}, snapshot => subscriber.next(snapshot), error => subscriber.error(error));
            return () => unsubscribe();
        }
